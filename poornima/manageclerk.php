...
<div style = "position:relative;top:-445px;color:#34282C;font-size:30px;"><center><b>Manage Clerk </b></center></div>
<table  style="position:absolute;top:300px;">
<tr>
	<th class = "col2" style = "background-color : goldenrod">Clerk ID</th>
	<th class = "col2" style = "background-color : goldenrod">Clerk Name</th>
...
if($result->num_rows>0)
{
	...
	{
			echo "<tr><td>".$row["clerkID"]."</td><td>".$row["clerkname"]."</td><td>".$row["email"]."</td><td>".$row["mobile"]."</td>";
			
			
			
			
			echo "<td><a href='editclerk.php?id=$row[clerkID]&name=$row[clerkname]&email=$row[email]&mobile=$row[mobile]&psw=$row[password]'><input type='submit' value='Edit'/></a></td>";
			
			echo "<td><a href='deleteclerk.php?id=$row[clerkID]&name=$row[clerkname]&email=$row[email]&mobile=$row[mobile]&psw=$row[password]'><input type='submit' value='Delete'/></a></td></tr>";
